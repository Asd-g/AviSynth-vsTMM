...
void CreateMM::threshMask_avx512(PVideoFrame& src, PVideoFrame& dst, int plane, IScriptEnvironment* env) noexcept
{
    switch (plane)
    {
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x += step)
        {
            ...
            T2 min0 = peak, max0 = _mm512_setzero_si512();
            T2 min1 = peak, max1 = _mm512_setzero_si512();
...
template void CreateMM::threshMask_avx512<uint8_t, Vec64uc, 64>(PVideoFrame& src, PVideoFrame& dst, int plane, IScriptEnvironment* env) noexcept;
template void CreateMM::threshMask_avx512<uint16_t, Vec32us, 32>(PVideoFrame& src, PVideoFrame& dst, int plane, IScriptEnvironment* env) noexcept;

template<typename T1, typename T2, int step>
void CreateMM::motionMask_avx512(PVideoFrame& src1, PVideoFrame& msk1, PVideoFrame& src2, PVideoFrame& msk2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x += step)
        {
            ...
            select(diff <= threshq, T2(1), _mm512_setzero_si512()).store_nt(dstpq + x);
            select(diff <= threshh, T2(1), _mm512_setzero_si512()).store_nt(dstph + x);
        }
...
template void CreateMM::motionMask_avx512<uint8_t, Vec64uc, 64>(PVideoFrame& src1, PVideoFrame& msk1, PVideoFrame& src2, PVideoFrame& msk2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;
template void CreateMM::motionMask_avx512<uint16_t, Vec32us, 32>(PVideoFrame& src1, PVideoFrame& msk1, PVideoFrame& src2, PVideoFrame& msk2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;

template<typename T1, typename T2, int step>
void CreateMM::andMasks_avx512(PVideoFrame& src1, PVideoFrame& src2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept
{
    const int width = vi.width >> (plane != PLANAR_Y ? vi.GetPlaneWidthSubsampling(PLANAR_U) : 0);
...
template void CreateMM::andMasks_avx512<uint8_t, Vec64uc, 64>(PVideoFrame& src1, PVideoFrame& src2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;
template void CreateMM::andMasks_avx512<uint16_t, Vec32us, 32>(PVideoFrame& src1, PVideoFrame& src2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;

template<typename T1, typename T2, int step>
void CreateMM::combineMasks_avx512(PVideoFrame& src, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x += step)
        {
            ...
            select(T2().load_a(srcp0 + x) == T2(_mm512_setzero_si512()) && T2().load_a(srcp1 + x) != T2(_mm512_setzero_si512()) && count >= cstr_, peak, T2().load_a(dstp + x)).store_nt(dstp + x);
        }
...
template void CreateMM::combineMasks_avx512<uint8_t, Vec64uc, 64>(PVideoFrame& src, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;
template void CreateMM::combineMasks_avx512<uint16_t, Vec32us, 32>(PVideoFrame& src, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept;
